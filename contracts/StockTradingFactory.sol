...
contract StockTradingFactory is SepoliaConfig {
    ...
    function createStockToken(
        ...
        StockToken newStockToken = new StockToken(stockName, tokenName, tokenSymbol, initialPrice, address(this));
    ...
    function updateStockPrice(string memory stockName, uint256 newPrice) external {
        address tokenAddress = stockTokens[stockName];
        if (tokenAddress == address(0)) revert StockNotFound();
    ...
    function mintStockTokens(string memory stockName, address to, uint64 amount) external {
        address tokenAddress = stockTokens[stockName];
        if (tokenAddress == address(0)) revert StockNotFound();
